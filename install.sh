...
"
    ...
    local IP=$(curl -s -m 2 ifconfig.me 2>/dev/null || curl -s -m 2 icanhazip.com 2>/dev/null || hostname -I 2>/dev/null | awk '{print $1}' || echo "localhost")
    if [ "$TARGET" = "main" ]; then
        log_success "Verse Panel (Main) installation is complete and verified!"
        echo -e "${GREEN}✓ You can now open http://${IP}:6767 and log in with '${OWNER_USER}'.${NC}
"
    else
        log_success "Verse Panel (Developer) installation is complete and verified!"
...
while true; do
    ...
    echo -e "  ${BOLD}3)${NC} Update Verse Panel"
    echo -e "  ${BOLD}4)${NC} Create Owner"
    echo -e "  ${BOLD}5)${NC} Uninstall Verse Panel"
